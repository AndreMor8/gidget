...
export default class extends SlashCommand {
  ...
  async run(bot, interaction) {
    ...
    const emoji = (pre_emoji ? (interaction.guild.emojis.cache.get(pre_emoji)?.identifier || interaction.guild.emojis.cache.find(e => e.name === pre_emoji || e.toString() === pre_emoji)?.identifier || (/(\u00a9|\u00ae|[\u2000-\u3300]|\ud83c[\ud000-\udfff]|\ud83d[\ud000-\udfff]|\ud83e[\ud000-\udfff])/.test(pre_emoji) ? pre_emoji : undefined)) : undefined);
    ...
    try {
      const embed = new Discord.MessageEmbed()
        ...
        .setDescription(`Press ${emoji ? (/(\u00a9|\u00ae|[\u2000-\u3300]|\ud83c[\ud000-\udfff]|\ud83d[\ud000-\udfff]|\ud83e[\ud000-\udfff])/.test(emoji) ? emoji : `<${(emoji.startsWith("a") ? "" : ":") + emoji}>`) : "*no emoji*"} to create a ticket.'`)
        .setColor("RANDOM")
