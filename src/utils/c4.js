...
export function displayConnectFourBoard(mapa, game) {
    ...
    return new Promise(async (s) => {
        const win = await Canvas.loadImage(path.join(__dirname, "/morado_de_4.png"))
        const bck = await Canvas.loadImage(path.join(__dirname, "/4enraya.png"))
        const imgs = {
            "🟢": await Canvas.loadImage(path.join(__dirname, "/rojo_de_cuatro.png")),
            "🟡": await Canvas.loadImage(path.join(__dirname, "/amarillo_de_cuatro.png"))
        }
