export default class extends Command {
  ...
  async run(bot, message, args) {
    ...
    if (channel) await channel.edit({ rateLimitPerUser: number, reason: "Slowmode command" }).then(() => { message.channel.send('Slowmode changed to ' + number + ' seconds in ' + message.channel.toString()); }).catch(err => message.reply(`I couldn't change the slowmode. Here's a debug: ` + err));
