  ...
  async run(bot, message, args) {
    switch (args[1]) {
      case 'global': {
        ...
        const embed = new EmbedBuilder().setTitle(`Connect4 Global Top Winners (${difficulty})`);
        for (const info of toshow) embed.addFields([{ name: info.cacheName || '?', value: `**Wins:** ${info.wins}\t**Loses:** ${info.loses}` }])
      ...
      default: {
        ...
        const embed = new EmbedBuilder().setTitle(`Connect4 Top Winners (${difficulty})`);
        for (const info of toshow) embed.addFields([{ name: info.cacheName || '?', value: `**Wins:** ${info.wins}\t**Loses:** ${info.loses}` }]);
        if (!args[1]) embed.setAuthor({ name: `@${bot.user.username} c4top [server/global] [difficulty]` });
        message.channel.send({ embeds: [embed] });
      }
    }
  }
