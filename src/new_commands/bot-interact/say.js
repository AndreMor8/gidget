...
export default class extends SlashCommand {
  ...
  async run(bot, interaction) {
    ...
    if (typeof interaction.options.getBoolean("button", false) === "boolean") {
      if (!interaction.member.permissions.has(8n)) return interaction.reply({ content: "You aren't admin.", ephemeral: true });
    ...
    return interaction.reply({ content: "Usage: `/say to-say:<text>`", ephemeral: true });
  }
}