    ...
    async run(bot, message, args) {
        ...
        switch (args[1]) {
            case 'add': {
                ...
                const channel = message.mentions.channels.filter(e => e.guild.id === message.guild.id && e.type === "news").first() || message.guild.channels.cache.get(args[2]);
            ...
            case 'delete': {
                if (!args[2]) message.channel.send("Please put a channel!");
                let channel = message.mentions.channels.filter(e => e.guild.id === message.guild.id && e.type === "news").first() || message.guild.channels.cache.get(args[2]);
                if (!channel) {
