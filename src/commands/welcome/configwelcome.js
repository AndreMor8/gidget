...
module.exports = {
    ...
    guildonly: true,
    description: "Welcome and goodbye system",
    run: async (bot, message, args) => {
        ...
        switch (args[1].toLowerCase()) {
            case "enable": {
                ...
                if(!channel.permissionsFor(message.guild.me).has(["VIEW_CHANNEL", "SEND_MESSAGES"])) return message.channel.send("Give me permissions for send messages on the established channel before starting the welcome system.")
                const reference = !!doc.enabled
                await message.guild.setWelcome(0, !reference)
                message.channel.send("The welcome system has been " + (!reference ? "Enabled" : "Disabled"))
            }
            break;
            case "channel": {
                ...
                if(channel.type !== "text" && channel.type !== "news") return message.channel.send("That isn't a text-based channel");
                if(!channel.permissionsFor(message.guild.me).has(["VIEW_CHANNEL", "SEND_MESSAGES"])) return message.channel.send("I don't have permissions for send messages in that channel");
                await message.guild.setWelcome(1, channel.id);
                message.channel.send("Channel set correctly");
            }
            break;
            case "message": {
                if(!args[2]) return message.channel.send("You have not put anything");
                if(/%INVITER%/gmi.test(args.slice(2).join(" "))) {
                    if(!message.guild.me.hasPermission("MANAGE_GUILD")) return message.channel.send("You must give me the permission to manage guild if you want the who invited the user to appear");
                }
                await message.guild.setWelcome(2, args.slice(2).join(" "))
                message.channel.send("Welcome message set correctly");
            }
            break;
            case "dmenable": {
                ...
                message.channel.send("The DM welcome system has been " + (!reference ? "Enabled" : "Disabled"));
            }
            break;
            case "dmmessage": {
                if(!args[2]) return message.channel.send("You have not put anything");
                ...
                message.channel.send("DM message set correctly");
            }
            break;
            case "leaveenable": {
                ...
                if(!channel) return message.channel.send("The established channel doesn't exist.");
                if(!channel.permissionsFor(message.guild.me).has(["VIEW_CHANNEL", "SEND_MESSAGES"])) return message.channel.send("Give me permissions for send messages on the established channel before starting the goodbye system.")
                const reference = !!doc.leaveenabled
                await message.guild.setWelcome(5, !reference)
                message.channel.send("The goodbye system has been " + (!reference ? "Enabled" : "Disabled"))
            }
            break;
            case "leavechannel": {
                if(!args[2]) return message.channel.send("You have not put a channel");
            ...
            case "leavemessage": {
                ...
                message.channel.send("Goodbye message set correctly");
            }
            break;
            default:
                message.channel.send("Invalid mode!");
        }
        if(message.guild.me.hasPermission("MANAGE_GUILD")) message.guild.inviteCount = await message.guild.getInviteCount().catch(err => {return {}});
    }
}