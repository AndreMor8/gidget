...
export default class extends Command {
    ...
    async run(bot, message) {
        ...
        const pings = [`📨 Message ping: ${Date.now() - msg.createdTimestamp}ms`, `📡 Ping from the API: ${(await bot.shard.fetchClientValues("ws.ping")).reduce((a, c, i, arr) => i === (arr.length - 1) ? (a + c) / arr.length : a + c, 0)}ms`, `📦 Shard ${bot.shard?.ids[0] || 0} ping: ${bot.ws.ping || "?"}ms`];
